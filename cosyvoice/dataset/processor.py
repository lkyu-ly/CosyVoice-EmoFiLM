...
def trim_token_mel(sample, token_mel_ratio=0):
    """按 token/mel 比例裁剪 speech token 与 mel，保持两者联合对齐。"""
    if not token_mel_ratio:
        return sample
    if token_mel_ratio < 1:
        raise ValueError("token_mel_ratio must be zero or a positive integer")
    speech_feat = sample['speech_feat']
    speech_token = sample['speech_token']
    token_len = min(
        speech_feat.shape[0] // token_mel_ratio,
        len(speech_token),
    )
    sample['speech_feat'] = speech_feat[:token_len * token_mel_ratio]
    sample['speech_token'] = speech_token[:token_len]
    return sample
...
def compute_fbank(data,
    ...
    for sample in data:
        ...
        trim_token_mel(sample, token_mel_ratio=token_mel_ratio)
        yield sample
