"""Emo-FiLM 训练工具: 冻结策略 + optimizer 分组。
...
def freeze_all_except(model, modules_to_unfreeze):
    ...
    for name, p in bare_model.named_parameters():
        p.requires_grad = any(
            _matches_module_name(name, tm) for tm in modules_to_unfreeze
        )
...
def init_optimizer_emo(model, configs):
    ...
    bare_model = _unwrap_model(model)
    emotion_module_names = {"emotion_encoder", "emotion_adapter", "emotion_classifier", "llm_decoder"}
