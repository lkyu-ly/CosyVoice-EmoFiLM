...
def main():
    ...
    # === Emo-FiLM 扩展：在 wrap_cuda_model 之前调用 freeze_all_except（裸 Module）===
    # spec 9.4: 仅解冻 emotion_* 和 llm_decoder，其它参数冻结
    ...
    # === Emo-FiLM 扩展：替换 init_optimizer_and_scheduler 为 emo 版本 ===
    # init_optimizer_emo 按 new_params_lr (emotion 模块) vs lr (base) 分组
    # 保留 5 元组返回接口（与父类调用约定一致），scheduler 用 ConstantLR 占位
    optimizer = init_optimizer_emo(model, configs)
    scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer)
    if gan is True:
        optimizer_d = torch.optim.Adam([p for p in model.parameters() if p.requires_grad], lr=1e-6)
    else:
        optimizer_d = None
    scheduler_d = torch.optim.lr_scheduler.ConstantLR(optimizer_d) if optimizer_d is not None else None
