...
覆盖（brief 06 DoD）：
  ...
  - v2 模型仍无 ``emotion_classifier``；新增 ``emotion_head``/``arousal_head``
    为随机初始化可训练 Linear（5 类 / 标量回归）；
  - base loader 允许 emotion_head/arousal_head（+ FiLM）缺失于 base ckpt；
    trained loader 严格加载；
  - v1 基线锚 git ``9c6d84b``（ADR-0020 扁平化后不再用源码 sha256 锁）；
    测试改为反转语义（断言 ``emotion_classifier`` 等反模式已删）+
    ``llm_emotion.py`` 活跃主线代码可演化。
...
# 反转语义锁（ADR-0020）：v1 输入端 classifier 反模式已从活跃 llm_emotion.py 删除。
# 源码 sha256 锁已删除（禁源码哈希标定；v1 基线由 git 锚点 ``9c6d84b`` 保证）。
...
# ============================================================
# A. 模型结构：heads 存在 / 无输入端 classifier
# ============================================================


def test_model_has_downstream_heads_and_no_input_classifier():
    model = _make_model()
    # 新增下游任务头
    ...
    # v2 仍无输入端 classifier / criterion_emotion_cls / emo_loss_weight
...
def test_init_accepts_head_weights_distinct_from_v1():
    ...
    # 语义不同于 v1 输入端 emo_loss_weight（死字段）
    assert "emo_loss_weight" not in params
...
def test_anti_shortcut_changing_target_does_not_change_head_input():
    ...
    assert not torch.allclose(out_a["loss_emotion"], out_b["loss_emotion"])
...
def test_emotion_mask_false_contributes_no_emotion_loss():
    ...
    assert float(out["loss_emotion"]) == 0.0
...
def test_soft_distribution_loss_matches_manual_soft_ce():
    ...
    torch.testing.assert_close(out["loss_emotion"], expected.detach())
...
def test_hard_ce_one_hot_is_special_case_of_soft_ce():
    ...
    torch.testing.assert_close(out["loss_emotion"], manual_ce.detach())
...
def test_v1_input_classifier_removed_from_active_code():
    """v1 输入端 ``emotion_classifier`` 反模式已从活跃 llm_emotion.py 删除。

    ADR-0020 禁源码哈希标定，改为断言反模式不存在。v1 基线由 git 锚点
    ``9c6d84b`` 保证。
    """
    src = ACTIVE_LLM_EMOTION_PATH.read_text(encoding="utf-8")
    assert "self.emotion_classifier = nn.Linear" not in src
    # 但下游 emotion_head / arousal_head 任务头保留（反捷径监督）
    assert "self.emotion_head = nn.Linear" in src
    assert "self.arousal_head = nn.Linear" in src
...
def test_active_config_carries_head_weights_not_dead():
    ...
    # 与 v1 输入端 emo_loss_weight 语义不同；不得混用死字段
    for dead in ("emo_loss_weight",):
        # yaml 顶层或 llm 块缩进下都不得出现该死字段
